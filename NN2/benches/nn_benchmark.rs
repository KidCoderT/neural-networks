...
use nn2::nn::benchmark::{emtpy_new_nn, nn_output, test_old, train_new_nn};
...
pub fn nn_benchmarks(c: &mut Criterion) {
    ...
    untrained_nn.bench_function("NEW", |b| b.iter(|| emtpy_new_nn()));
    ...
    trained_nn.bench_function("NEW", |b| {
        b.iter(|| {
            train_new_nn(
                [[0.31386864, 0.5474453], [-0.6496351, -0.021897793]],
                [-0.44525546, 0.0],
            )
        })
    });
    ...
    c.bench_function("NN Output Function", |b| b.iter(|| nn_output()));
}
